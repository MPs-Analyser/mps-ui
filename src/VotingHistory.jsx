...
function VotingHistory({ votingHistory, onQueryDivision }) {
  ...
  const columns = [
    columnHelper.accessor('icon', {
      cell: info => (
        <span onClick={() => onQueryDivision(info.row.original.divisionId)}>
    ...
    columnHelper.accessor('date', {
      cell: info => info.getValue() ? <span onClick={() => onQueryDivision(info.row.original.divisionId)} style={{ textWrap: 'nowrap' }}>{info.getValue().substring(0,10)}</span> : '',      
      header: <span style={{ position: 'relative', left: -30, marginRight: 12 }}>Date</span>
    }),
